...
def build_val_test_dl(config, data):
    root_config = config
    local_config = root_config['eval']
    
    if 'not_eval' in local_config and local_config['not_eval']:
        return None, None
    ...
    validation_method = local_config['validation_method']
    test_method = local_config['test_method']
    
    assert validation_method in all_eval_method
    assert test_method in all_eval_method
    
    val_batch_size = local_config['val_batch_size'] if 'val_batch_size' in local_config else 100
    test_batch_size = local_config['test_batch_size'] if 'test_batch_size' in local_config else 100
    
    val_dl = build_eval_dl(validation_method, 
                           local_config['file_validation'], 
                           local_config['mask_nei_when_validation'], 
                           val_batch_size,
                           data)
    
    test_dl = build_eval_dl(test_method, 
                            local_config['file_test'],
                            local_config['mask_nei_when_test'],
                            test_batch_size,
                            data)
